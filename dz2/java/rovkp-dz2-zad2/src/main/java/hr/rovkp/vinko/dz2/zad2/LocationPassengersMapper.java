...
public class LocationPassengersMapper extends Mapper<LongWritable, Text, IntWritable, Text> {
    ...
    public final static String HEADER_PARAMETER = "SKIP_HEADERS";

    @Override
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        
        
        if (!context.getConfiguration().getBoolean(HEADER_PARAMETER, true) || key.get() > 0) {
